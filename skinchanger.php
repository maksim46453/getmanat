...
require_once __DIR__ . '/includes/safe_database.php';
...
$db = new SafeDatabase();
$dbError = !$db->isConnected() ? ($db->getConnectionError() ?? 'Database unavailable.') : '';
$skinError = '';
$weapons = [];
$skins = [];
$selectedSkins = [];
...
<main class="container py-4">
    <?php if (!isset($_SESSION['steamid'])): ?>
        <section class="portal-card text-center">
            ...
            <p>Увійдіть через Steam для синхронізації скінів з сервером.</p>
    ...
    <?php else: ?>
        <section class="portal-card mb-3 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            ...
            <div class="d-flex gap-2">
                <a class="btn btn-outline-light" href="<?php echo e($_SERVER['PHP_SELF']); ?>?update">Оновити профіль</a>
                <a class="btn btn-danger" href="<?php echo e($_SERVER['PHP_SELF']); ?>?logout">Logout</a>
            </div>
        </section>

        <?php if ($dbError !== ''): ?>
            <div class="alert alert-warning">SkinChanger тимчасово недоступний: немає підключення до БД. Перевір `class/config.php`.</div>
        <?php endif; ?>
        <?php if ($skinError !== ''): ?>
        ...
        <div class="row g-3">
            <div class="col-xl-3 col-lg-4 col-md-6">
                <div class="portal-card h-100">
                    <?php
                    $actualKnife = $knifes[0] ?? ['paint_name' => 'Default knife', 'image_url' => ''];
                    ...
                    <form action="" method="POST" class="mt-3">
                        ...
                        <select name="forma" class="form-select" onchange="this.form.submit()">
                            ...
                            <?php foreach ($knifes as $knifeKey => $knife): ?>
                                <option <?php echo (($selectedKnife[0]['knife'] ?? '') === $knife['weapon_name']) ? 'selected' : ''; ?> value="knife-<?php echo e((string) $knifeKey); ?>"><?php echo e($knife['paint_name']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </form>
                </div>
                        ...
                        <form action="" method="POST" class="mt-3 mt-auto">
                            ...
                            <select name="forma" class="form-select" onchange="this.form.submit()">
                                ...
                                <?php foreach ($skins[$defindex] as $paintKey => $paint): ?>
                                    <option <?php echo (array_key_exists($defindex, $selectedSkins) && $selectedSkins[$defindex]['weapon_paint_id'] == $paintKey) ? 'selected' : ''; ?> value="<?php echo e((string) $defindex); ?>-<?php echo e((string) $paintKey); ?>"><?php echo e($paint['paint_name']); ?></option>
                                <?php endforeach; ?>
                            </select>
                            <?php $selectedSkinInfo = $selectedSkins[$defindex] ?? null; $initialWearValue = $selectedSkinInfo['weapon_wear'] ?? 1.0; $initialSeedValue = $selectedSkinInfo['weapon_seed'] ?? 0; ?>
                            ...
                            <div class="modal fade" id="weaponModal<?php echo e((string) $defindex); ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog"><div class="modal-content bg-dark text-light"><div class="modal-header"><h5 class="modal-title"><?php echo e((array_key_exists($defindex, $selectedSkins) ? $skins[$defindex][$selectedSkins[$defindex]['weapon_paint_id']]['paint_name'] : $default['paint_name']) . ' Settings'); ?></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="mb-3"><label class="form-label">Wear preset</label><select class="form-select" onchange="document.getElementById('wear<?php echo e((string) $defindex); ?>').value = this.value;"><option disabled>Select Wear</option><option value="0.00" <?php echo ($initialWearValue == 0.00) ? 'selected' : ''; ?>>Factory New</option><option value="0.07" <?php echo ($initialWearValue == 0.07) ? 'selected' : ''; ?>>Minimal Wear</option><option value="0.15" <?php echo ($initialWearValue == 0.15) ? 'selected' : ''; ?>>Field-Tested</option><option value="0.38" <?php echo ($initialWearValue == 0.38) ? 'selected' : ''; ?>>Well-Worn</option><option value="0.45" <?php echo ($initialWearValue == 0.45) ? 'selected' : ''; ?>>Battle-Scarred</option></select></div><div class="row g-3"><div class="col-6"><label class="form-label" for="wear<?php echo e((string) $defindex); ?>">Wear</label><input type="text" class="form-control" id="wear<?php echo e((string) $defindex); ?>" name="wear" value="<?php echo e((string) $initialWearValue); ?>"></div><div class="col-6"><label class="form-label" for="seed<?php echo e((string) $defindex); ?>">Seed</label><input type="text" class="form-control" id="seed<?php echo e((string) $defindex); ?>" name="seed" value="<?php echo e((string) $initialSeedValue); ?>" oninput="this.value=this.value.replace(/[^0-9]/g, '').slice(0,4)"></div></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button><button type="submit" class="btn btn-danger">Use</button></div></div></div>
                            </div>
                        </form>
                    </div>
                </div>
